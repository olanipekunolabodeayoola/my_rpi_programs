...
//
// Difference(s):
// (1.)To modify class CleanupGPIOPin to set and get GPIO
// pin numbers according to wiringPi numbering system
// for the purpose of clean up (see 101.lv)
//
// (2.)
//
//
...
typedef const unsigned short int CUSHORT;    // typedef defined (See 101.lv)
...
class CleanupGPIOPin
{
    public:
        ...
        unsigned long int GetPinNumber();    // "
        void setPinNum(unsigned long int pinNum_001, pinNum_002);    // "
    private:
        // Member data:
        volatile sig_atomic_t blinkState;            // blink status/condition
        CUSHORT totalPins;                   // total number of pins
                                             // (NB: size = 2 bytes (See 101.lv)
        ULONG myPinNumber[totalPins-1];      // " (NB: Array size =
...
CleanupGPIOPin::CleanupGPIOPin()
{
    ...
    totalPins = 2;               // WARNING: Watch out for errors since
                                 // 'totalPins' is a constant variable type
    myPinNumber[] = {29,28};     //
}
...
// GetPinNumber public accessor fxn  definition:
unsigned long int CleanupGPIOin::GetPinNumber()
{
    return myPinNumber
}
